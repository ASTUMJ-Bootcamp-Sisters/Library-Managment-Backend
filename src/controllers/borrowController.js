...
async function borrowBook(req, res) {
  try {
    const { bookId, duration } = req.body; // duration instead of raw dueDate
    const studentId = req.user.id; // assume user is authenticated (from middleware)
...
async function returnBook(req, res) {
  try {
    ...
    const borrow = await Borrow.findOne({ _id: borrowId, student: studentId }).populate("book");
